...
curl -s -X POST -b "$COOKIES_FILE" \
  ...
  -H "Content-Type: application/json" \
  -d '{"name":"PEAC","metadata":{"dc.title":[{"value":"Programa de Educación de Adultos por Correspondencia"}],"dc.description":[{"value":"Modalidad de educación a distancia dirigida a jóvenes y adultos que desean completar la educación primaria mediante materiales autoinstructivos. Incluye guías de estudio, evaluaciones y recursos pedagógicos del programa PEAC."}],"dc.subject":[{"value":"PEAC"}],"dc.type":[{"value":"menu-principal"}],"dc.format":[{"value":"documento"}],"dc.identifier.other":[{"value":"1"}]}}' \
...
curl -s -X POST -b "$COOKIES_FILE" \
  -H "Authorization: Bearer $JWT" -H "X-XSRF-TOKEN: $CSRF_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"Modalidades Flexibles","metadata":{"dc.title":[{"value":"Programa Modalidades Flexibles para la Educación Media"}],"dc.description":[{"value":"Programa de educación media con metodologías flexibles dirigido a jóvenes y adultos de 15 años en adelante. Atiende ciclo básico y diversificado mediante modalidades semipresenciales y a distancia. Incluye recursos educativos, lineamientos metodológicos y materiales de apoyo adaptados a las necesidades de los estudiantes."}],"dc.subject":[{"value":"Modalidades Flexibles"}],"dc.type":[{"value":"menu-principal"}],"dc.format":[{"value":"documento"}],"dc.identifier.other":[{"value":"2"}]}}' \
  "$BASE_URL/api/core/collections?parent=$ED_BASICA_UUID" > /dev/null
...
curl -s -X POST -b "$COOKIES_FILE" \
  -H "Authorization: Bearer $JWT" -H "X-XSRF-TOKEN: $CSRF_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"PRONEA","metadata":{"dc.title":[{"value":"Programa Nacional de Educación Alternativa"}],"dc.description":[{"value":"Programa de alfabetización y educación básica para población adulta mediante metodologías flexibles y contextualizadas. Contiene materiales didácticos, manuales para facilitadores y documentación del programa PRONEA."}],"dc.subject":[{"value":"PRONEA"}],"dc.type":[{"value":"menu-principal"}],"dc.format":[{"value":"documento"}],"dc.identifier.other":[{"value":"3"}]}}' \
  "$BASE_URL/api/core/collections?parent=$ED_BASICA_UUID" > /dev/null
log_success "  PRONEA"
...
curl -s -X POST -b "$COOKIES_FILE" \
  -H "Authorization: Bearer $JWT" -H "X-XSRF-TOKEN: $CSRF_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"Investigaciones","metadata":{"dc.title":[{"value":"Investigaciones Educativas"}],"dc.description":[{"value":"Estudios, investigaciones y análisis sobre educación extraescolar en Guatemala. Incluye investigaciones propias, tesis, estudios de caso, diagnósticos y documentos de investigación educativa."}],"dc.type":[{"value":"menu-secundario"}],"dc.format":[{"value":"documento"}]}}' \
...
curl -s -X POST -b "$COOKIES_FILE" \
  -H "Authorization: Bearer $JWT" -H "X-XSRF-TOKEN: $CSRF_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"Informes de Gestión","metadata":{"dc.title":[{"value":"Informes de Gestión y Memorias Institucionales"}],"dc.description":[{"value":"Informes anuales, memorias de labores, planes operativos anuales (POA), rendición de cuentas y documentación de gestión administrativa de DIGEEX."}],"dc.type":[{"value":"menu-secundario"}],"dc.format":[{"value":"documento"}]}}' \
  "$BASE_URL/api/core/collections?parent=$ED_INVESTIGACION_UUID" > /dev/null
